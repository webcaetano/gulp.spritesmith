...
function gulpSpritesmith(params) {
  ...
  var onEnd = function (cb) {
    ...
    if(!!params.numericSort) images = images.sort(basenameSort);
    ...
    async.parallel([
      ...
      Object.getOwnPropertyNames(coordinates).sort(basenameSort).forEach(function (file) {
      ...
      if (retinaResult) {
        ...
        // Clean up the file name of the file
        Object.getOwnPropertyNames(retinaCoordinates).sort(basenameSort).forEach(function prepareRetinaTemplateData (file) {
          var name = getCoordinateName(file);
